...
import { ArrowLeft, Search, RefreshCw, ExternalLink, TrendingUp, TrendingDown, X, Info } from 'lucide-react'
    ...
    >
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          <h3 className="text-white font-medium text-sm mb-1 line-clamp-2">{event.title}</h3>
          <div className="flex items-center gap-2 text-xs">
            <span className="text-white/40">{formatVolume(event.volume)} vol</span>
            {event.markets?.length > 1 && (
              <span className="text-purple-400">{event.markets.length} markets</span>
            )}
          </div>
        </div>
        <div className="flex flex-col items-end">
          <span className={`text-lg font-bold ${
            yesPrice >= 0.7 ? 'text-green-400' : yesPrice <= 0.3 ? 'text-red-400' : 'text-yellow-400'
          }`}>
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <span className={`text-lg font-bold ${
          parsed.yesPrice >= 0.7 ? 'text-green-400' : parsed.yesPrice <= 0.3 ? 'text-red-400' : 'text-yellow-400'
        }`}>
...
function TradingPanel({ market, onClose }: { market: PolymarketMarket; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-[430px] bg-[#0a0a0a] border-t border-white/[0.1] rounded-t-3xl p-5 max-h-[85vh] overflow-y-auto">
        {/* Handle */}
        ...
        <div className="flex gap-2 mb-4">
          <button
            ...
            className={`flex-1 py-3 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 ${
              side === 'YES' ? 'bg-green-500 text-white' : 'bg-white/[0.05] text-white/60'
            }`}
          >
          ...
          <button
            onClick={() => setSide('NO')}
            className={`flex-1 py-3 rounded-xl font-semibold transition-all flex items-center justify-center gap-2 ${
              side === 'NO' ? 'bg-red-500 text-white' : 'bg-white/[0.05] text-white/60'
            }`}
          >
          ...
          <div className="flex gap-2 mt-3">
            {[5, 10, 25, 50].map((amt) => (
              <button
                ...
                className="flex-1 py-1.5 bg-white/[0.05] hover:bg-white/[0.08] rounded-lg text-white/60 text-xs"
              >
                ${amt}
              </button>
            ))}
        ...
        <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3 mb-4">
          <div className="flex items-start gap-2">
            ...
            <p className="text-purple-400 text-xs">
              Polymarket trades on Polygon. Bridge USDC from Base to trade.
            </p>
          </div>
        </div>
