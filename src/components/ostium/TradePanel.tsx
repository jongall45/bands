...
export function OstiumTradePanel({ pair }: TradePanelProps) {
  ...
  const { openTrade, isPending, isSuccess, isSwitchingChain, error, txHash, callsStatus, reset } = useOstiumTrade()
      ...
      >
        {isPending ? (
          <>
            ...
            {isSwitchingChain ? 'Switching to Arbitrum...' : 'Opening Position...'}
          </>
        ) : !isConnected ? (
          'Connect Wallet'
        ) : !hasEnoughGas ? (
